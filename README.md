...
without cross-compilation, and verification — lives in
...
`pkgs.opencode`, `pkgs.opencode-desktop` and `pkgs.pi-coding-agent`. Note that
`pkgs.opencode`, `pkgs.opencode-desktop` and `pkgs.pi-coding-agent` replace the
nixpkgs versions.
