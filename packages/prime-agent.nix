...
buildNpmPackage (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-j1WMuSIjNA9gy40kAqz+GN6WzKSh8vJisfOmwfSs2mA=";
  ...
  patches = [
    ...
    ./prime-agent-remove-uv-python-install.patch
  ];

  npmDepsFetcherVersion = 2;
  npmDepsHash = "sha256-40XSXO3pJPpjAFTI2PtOpUgcctefJlKd+UYhRvR5qoc=";
  ...
  installCheckPhase = ''
    ...
    requiredPythonVersion=$(sed -n 's/.*PYTHON_VERSION = "\([^"]*\)".*/\1/p' \
      packages/coding-agent/src/core/kernel/bootstrap.ts | head -n 1)
